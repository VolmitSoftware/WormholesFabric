...
public class VoidPlacementHandler {
    ...
    private static void spawnVoidPlatform(World world, BlockPos pos) {
        if (world.getBlockState(pos).getBlock() != BlockRegistry.FRAME_BLACK) {
            BlockState platformBlock = BlockRegistry.FRAME_BLACK.getDefaultState();
            for (int x = -3; x < 4; x++) {
                for (int z = -3; z < 4; z++) {
            ...
            world.setBlockState(pos, BlockRegistry.FRAME_BLACK.getDefaultState());
